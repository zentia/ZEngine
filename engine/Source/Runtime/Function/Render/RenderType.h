...
// Shader stage -- non-bitflag, sequential enum used by the RHI
// createShaderModuleFromFile/FromSource API and all backend shader
// compilers.  This replaces the former `int` parameter that conflated
// two incompatible value spaces (sequential ShaderStage vs Vulkan-style
// RHIShaderStageFlagBits).
...
typedef uint32_t RHIDescriptorBindingFlags;

// -------------------------------------------------------------------------
// Bindless / descriptor-indexing flag bits
//   Mirror VkDescriptorBindingFlagBits / VkDescriptorPoolCreateFlagBits /
//   VkDescriptorSetLayoutCreateFlagBits relevant to bindless usage.
//   Default value 0 keeps existing (bindful) behaviour, so all current call
//   sites stay binary-compatible.
// -------------------------------------------------------------------------
constexpr RHIDescriptorBindingFlags RHI_DESCRIPTOR_BINDING_UPDATE_AFTER_BIND_BIT = 0x00000001;
constexpr RHIDescriptorBindingFlags RHI_DESCRIPTOR_BINDING_UPDATE_UNUSED_WHILE_PENDING_BIT = 0x00000002;
constexpr RHIDescriptorBindingFlags RHI_DESCRIPTOR_BINDING_PARTIALLY_BOUND_BIT = 0x00000004;
constexpr RHIDescriptorBindingFlags RHI_DESCRIPTOR_BINDING_VARIABLE_DESCRIPTOR_COUNT_BIT = 0x00000008;

constexpr RHIDescriptorPoolCreateFlags RHI_DESCRIPTOR_POOL_CREATE_UPDATE_AFTER_BIND_BIT = 0x00000002;
constexpr RHIDescriptorSetLayoutCreateFlags RHI_DESCRIPTOR_SET_LAYOUT_CREATE_UPDATE_AFTER_BIND_POOL_BIT = 0x00000002;
...
enum class ZENGINE_IMAGE_TYPE : uint8_t
{
    ZENGINE_IMAGE_TYPE_UNKNOWM = 0,
    ZENGINE_IMAGE_TYPE_2D
};
...
// Graphics API enumeration for multi-backend support
...
public:
    ...
    uint32_t m_PixelsLength {0};
    uint8_t* m_Pixels;

    RHIFormat m_Format = RHI_FORMAT_MAX_ENUM;
    ZENGINE_IMAGE_TYPE m_Type {ZENGINE_IMAGE_TYPE::ZENGINE_IMAGE_TYPE_UNKNOWM};
...
struct MeshSourceDesc
{
    ...
    bool operator==(const MeshSourceDesc& rhs) const { return m_MeshAsset == rhs.m_MeshAsset; }
    size_t getHashValue() const { return eastl::hash<eastl::string> {}(m_MeshAsset); }
};
...
struct MaterialSourceDesc
{
    ...
    eastl::string m_BaseColorFile;
    eastl::string m_MetallicRoughnessFile;
    eastl::string m_NormalFile;
    eastl::string m_OcclusionFile;
    eastl::string m_EmissiveFile;
    eastl::vector<eastl::string> m_EnabledShaderKeywords;

    bool operator==(const MaterialSourceDesc& rhs) const
    {
        return m_MaterialAsset == rhs.m_MaterialAsset && m_Shader == rhs.m_Shader &&
               m_BaseColorFile == rhs.m_BaseColorFile &&
               m_MetallicRoughnessFile == rhs.m_MetallicRoughnessFile && m_NormalFile == rhs.m_NormalFile &&
               m_OcclusionFile == rhs.m_OcclusionFile && m_EmissiveFile == rhs.m_EmissiveFile &&
               m_EnabledShaderKeywords == rhs.m_EnabledShaderKeywords;
    }
...
{
    std::shared_ptr<BufferData> m_VertexBuffer;
...
struct RenderMeshData
{
    ...
    std::shared_ptr<BufferData> m_SkeletonBindingBuffer;
};

struct RenderShaderPassData
{
...
struct RenderMaterialData
{
    ...
    eastl::string m_ShaderAssetFile;
    eastl::string m_VertexShaderFile;
    eastl::string m_FragmentShaderFile;
    eastl::string m_RenderPipeline {"StandardLit"};
    eastl::string m_LightMode {"GBuffer"};
    ...
    eastl::string m_VertexEntry {"main"};
    eastl::string m_FragmentEntry {"main"};
    eastl::string m_IncludeDirectory;
    eastl::string m_Cull {"Back"};
    eastl::string m_Ztest {"LEqual"};
    eastl::string m_Blend {"Off"};
    bool m_Zwrite {true};
    ...
    eastl::vector<eastl::string> m_EnabledShaderKeywords;
    std::vector<RenderShaderPassData> m_ShaderPasses;
    std::shared_ptr<TextureData> m_BaseColorTexture;
    std::shared_ptr<TextureData> m_MetallicRoughnessTexture;
    std::shared_ptr<TextureData> m_NormalTexture;
    std::shared_ptr<TextureData> m_OcclusionTexture;
    std::shared_ptr<TextureData> m_EmissiveTexture;
};

template<>
struct std::hash<MeshSourceDesc>
{
    size_t operator()(const MeshSourceDesc& rhs) const noexcept { return rhs.getHashValue(); }
};
template<>
struct std::hash<MaterialSourceDesc>
{
    size_t operator()(const MaterialSourceDesc& rhs) const noexcept { return rhs.getHashValue(); }
};
