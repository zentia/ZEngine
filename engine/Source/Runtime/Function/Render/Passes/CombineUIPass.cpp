...
void CombineUIPass::SetupDescriptorSetLayout()
{
    ...
    if (RHI_SUCCESS !=
        m_Rhi->CreateDescriptorSetLayout(&post_process_global_layout_create_info, m_DescriptorInfos[0].layout))
    {
                                                   ...
                                                   RHIImageView* ui_input_attachment)
{
    ...
    per_frame_scene_input_attachment_write_info.dstSet = m_DescriptorInfos[0].descriptor_set;
    ...
    per_frame_ui_input_attachment_write_info.dstSet = m_DescriptorInfos[0].descriptor_set;
...
void CombineUIPass::Draw()
{
    ...
    m_Rhi->CmdBindPipelinePFN(
        m_Rhi->GetCurrentCommandBuffer(), RHI_PIPELINE_BIND_POINT_GRAPHICS, m_RenderPipelines[0].pipeline);
