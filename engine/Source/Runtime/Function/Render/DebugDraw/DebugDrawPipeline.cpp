...
void DebugDrawPipeline::SetupRenderPass()
{
    ...
    debug_draw_dependency.srcStageMask =
        RHI_PIPELINE_STAGE_COLOR_ATTACHMENT_OUTPUT_BIT | RHI_PIPELINE_STAGE_EARLY_FRAGMENT_TESTS_BIT;
    debug_draw_dependency.dstStageMask =
        RHI_PIPELINE_STAGE_COLOR_ATTACHMENT_OUTPUT_BIT | RHI_PIPELINE_STAGE_EARLY_FRAGMENT_TESTS_BIT;
    debug_draw_dependency.srcAccessMask =
        RHI_ACCESS_COLOR_ATTACHMENT_WRITE_BIT | RHI_ACCESS_DEPTH_STENCIL_ATTACHMENT_WRITE_BIT;  // STORE_OP_STORE
    debug_draw_dependency.dstAccessMask =
        RHI_ACCESS_COLOR_ATTACHMENT_WRITE_BIT | RHI_ACCESS_DEPTH_STENCIL_ATTACHMENT_WRITE_BIT;
...
void DebugDrawPipeline::SetupDescriptorLayout()
{
    ...
    if (m_Rhi->CreateDescriptorSetLayout(&layoutInfo, m_DescriptorLayout) != RHI_SUCCESS)
    {
...
void DebugDrawPipeline::SetupPipelines()
{
    ...
    if (m_PipelineType == _debug_draw_pipeline_type_point)
    {
        input_assembly_create_info.topology = RHI_PRIMITIVE_TOPOLOGY_POINT_LIST;
    }
    else if (m_PipelineType == _debug_draw_pipeline_type_line)
    {
        input_assembly_create_info.topology = RHI_PRIMITIVE_TOPOLOGY_LINE_LIST;
    }
    else if (m_PipelineType == _debug_draw_pipeline_type_triangle)
    {
        input_assembly_create_info.topology = RHI_PRIMITIVE_TOPOLOGY_TRIANGLE_LIST;
    }
    else if (m_PipelineType == _debug_draw_pipeline_type_point_no_depth_test)
    {
        input_assembly_create_info.topology = RHI_PRIMITIVE_TOPOLOGY_POINT_LIST;
        depth_stencil_create_info.depthTestEnable = RHI_FALSE;
    }
    else if (m_PipelineType == _debug_draw_pipeline_type_line_no_depth_test)
    {
        input_assembly_create_info.topology = RHI_PRIMITIVE_TOPOLOGY_LINE_LIST;
        depth_stencil_create_info.depthTestEnable = RHI_FALSE;
    }
    else if (m_PipelineType == _debug_draw_pipeline_type_triangle_no_depth_test)
    {
        input_assembly_create_info.topology = RHI_PRIMITIVE_TOPOLOGY_TRIANGLE_LIST;
        depth_stencil_create_info.depthTestEnable = RHI_FALSE;
    }
