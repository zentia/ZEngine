...
// Forward declare Vulkan types - only include in Vulkan implementation
...
struct RHIDescriptorSetLayoutBinding
{
    ...
    // Bindless / descriptor-indexing per-binding flags.
    // Default 0 -> classic bindful behaviour. Set to a combination of
    // RHI_DESCRIPTOR_BINDING_* bits to opt this slot into bindless usage.
    RHIDescriptorBindingFlags bindingFlags = 0;
};
...
{
    RHIImage* depth_image = RHI_NULL_HANDLE;
    RHIImageView* depth_image_view = RHI_NULL_HANDLE;
...
struct QueueFamilyIndices
{
    ...
    bool IsComplete()
    {
        return graphics_family.has_value() && present_family.has_value() && m_ComputeFamily.has_value();
        ;
    }
};

// SwapChainSupportDetails is Vulkan-specific and moved to vulkan_rhi.h