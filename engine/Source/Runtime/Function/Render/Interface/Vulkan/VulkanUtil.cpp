...
std::unordered_map<uint32_t, VkSampler> VulkanUtil::m_MipmapSamplerMap;
VkSampler VulkanUtil::m_NearestSampler = VK_NULL_HANDLE;
VkSampler VulkanUtil::m_LinearSampler = VK_NULL_HANDLE;
                                    ...
                                    VkMemoryPropertyFlags properties_flag)
{
    ...
    LOG_ERROR(ZVulkan, "findMemoryType error");
    return 0;
}
                                           ...
                                           int datasize)
{
    ...
    {
        LOG_ERROR(ZVulkan, "create buffer buffer failed!");
        return;
    }
    ...
    if (!memTypeFound)
    {
        LOG_ERROR(ZVulkan, "memTypeFound is nullptr");
        return;
    }
    if (VK_SUCCESS != vkAllocateMemory(device, &memAlloc, nullptr, memory))
    {
        LOG_ERROR(ZVulkan, "alloc memory failed!");
        return;
    }
    ...
    if (data != nullptr && datasize != 0)
    {
        ...
        {
            LOG_ERROR(ZVulkan, "map memory failed!");
            return;
        }
    ...
    {
        LOG_ERROR(ZVulkan, "bind memory failed!");
        return;
    }
}
                              ...
                              VkDeviceMemory& buffer_memory)
{
    ...
    {
        LOG_ERROR(ZVulkan, "vkCreateBuffer failed!");
        return;
    }
    ...
    {
        LOG_ERROR(ZVulkan, "vkAllocateMemory failed!");
        return;
    }
                            ...
                            VkDeviceSize size)
{
    if (rhi == nullptr)
    {
        LOG_ERROR(ZVulkan, "rhi is nullptr");
        return;
    }
                             ...
                             uint32_t miplevels)
{
    ...
    {
        LOG_ERROR(ZVulkan, "failed to create image!");
        return;
    }
    ...
    {
        LOG_ERROR(ZVulkan, "failed to allocate image memory!");
        return;
    }
                                   ...
                                   uint32_t miplevels)
{
    ...
    switch (texture_image_format)
    {
        ...
        case RHIFormat::RHI_FORMAT_R32G32B32A32_SFLOAT:
            ...
            break;
        default:
            LOG_ERROR(ZVulkan, "invalid texture_byte_size");
            break;
    }
    ...
    VulkanUtil::CreateBuffer(static_cast<VulkanRHI*>(rhi)->m_PhysicalDevice,
    ...
    vmaCreateImage(static_cast<VulkanRHI*>(rhi)->m_AssetsAllocator,
                   &image_create_info,
                   &allocInfo,
                   &image,
                   &image_allocation,
    ...
    TransitionImageLayout(
        rhi, image, VK_IMAGE_LAYOUT_UNDEFINED, VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL, 1, 1, VK_IMAGE_ASPECT_COLOR_BIT);
                               ...
                               uint32_t miplevels)
{
    ...
    switch (texture_image_format)
    {
        ...
        default:
            ...
            LOG_ERROR(ZVulkan, "invalid texture_layer_byte_size");
            return;
            break;
    }
    ...
    VkBuffer inefficient_staging_buffer;
    VkDeviceMemory inefficient_staging_buffer_memory;
    CreateBuffer(static_cast<VulkanRHI*>(rhi)->m_PhysicalDevice,
                 static_cast<VulkanRHI*>(rhi)->m_Device,
                 cube_byte_size,
                 VK_BUFFER_USAGE_TRANSFER_SRC_BIT,
                 VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT | VK_MEMORY_PROPERTY_HOST_COHERENT_BIT,
                                        ...
                                        uint32_t miplevels)
{
    ...
    vkGetPhysicalDeviceFormatProperties(
        static_cast<VulkanRHI*>(rhi)->m_PhysicalDevice, image_format, &format_properties);
    if (!(format_properties.optimalTilingFeatures & VK_FORMAT_FEATURE_SAMPLED_IMAGE_FILTER_LINEAR_BIT))
    {
        LOG_ERROR(ZVulkan, "GenerateTextureMipMaps() : linear bliting not supported!");
        return;
    }
                                       ...
                                       VkImageAspectFlags aspect_mask_bits)
{
    ...
    {
        LOG_ERROR(ZVulkan, "unsupported layout transition!");
        return;
    }
...
VkSampler
VulkanUtil::GetOrCreateMipmapSampler(VkPhysicalDevice physical_device, VkDevice device, uint32_t width, uint32_t height)
{
    if (width <= 0 || height <= 0)
    {
...
VkSampler VulkanUtil::GetOrCreateNearestSampler(VkPhysicalDevice physical_device, VkDevice device)
{
    if (m_NearestSampler == VK_NULL_HANDLE)
    {
        ...
        samplerInfo.maxLod = 8.0f;  // todo: m_IrradianceTextureMiplevels
        samplerInfo.borderColor = VK_BORDER_COLOR_INT_OPAQUE_BLACK;
        samplerInfo.unnormalizedCoordinates = VK_FALSE;

        if (vkCreateSampler(device, &samplerInfo, nullptr, &m_NearestSampler) != VK_SUCCESS)
        {
            LOG_ERROR(ZVulkan, "vk create sampler");
        }
    }
