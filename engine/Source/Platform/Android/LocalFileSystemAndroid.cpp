#include "LocalFileSystemAndroid.h"
...
bool LocalFileSystemAndroid::Open(FileEntryData& data, FilePermission permissions, [[maybe_unused]] FileAutoBehavior behavior)
{
...
bool LocalFileSystemAndroid::Close(FileEntryData& data)
{
...
bool LocalFileSystemAndroid::Read(
    FileEntryData& data,
    FileSize from,
    uint64_t count,
    void* buffer,
...
bool LocalFileSystemAndroid::Write(FileEntryData& data, FileSize at, uint64_t size, const void* buffer, uint64_t* actual)
{
    const int fd = GetFileDescriptor(data);
    if (fd < 0 || buffer == nullptr || actual == nullptr)
    {
...
bool LocalFileSystemAndroid::Write(FileEntryData& data, uint64_t size, const void* buffer, uint64_t* actual)
{
    const int fd = GetFileDescriptor(data);
    if (fd < 0 || buffer == nullptr || actual == nullptr)
    {
...
FileSize LocalFileSystemAndroid::Size(const FileEntryData& data) const
{
    const int fd = GetFileDescriptor(data);
...
FileSystemError LocalFileSystemAndroid::GetLastFileSystemError(bool operationSuccessful) const
{
    return ConvertToFileSystemError(GetAndUpdateLastError(operationSuccessful));
}

FileSystemError LocalFileSystemAndroid::ConvertToFileSystemError(int error)
{
    switch (error)
    {
        case 0:
...
int LocalFileSystemAndroid::GetAndUpdateLastError(bool operationSuccessful) const
{
