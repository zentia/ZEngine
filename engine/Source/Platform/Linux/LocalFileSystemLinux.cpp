#include "LocalFileSystemLinux.h"
...
bool LocalFileSystemLinux::Open(FileEntryData& data, FilePermission permissions, [[maybe_unused]] FileAutoBehavior behavior)
{
...
bool LocalFileSystemLinux::Close(FileEntryData& data)
{
...
bool LocalFileSystemLinux::Read(
    FileEntryData& data,
    FileSize from,
    uint64_t count,
    void* buffer,
    ...
    *actual = 0;
    ...
    while (count > 0)
    {
        const size_t to_read =
            static_cast<size_t>(std::min<uint64_t>(count, static_cast<uint64_t>(std::numeric_limits<size_t>::max())));
...
bool LocalFileSystemLinux::Write(FileEntryData& data, FileSize at, uint64_t size, const void* buffer, uint64_t* actual)
{
    const int fd = GetFileDescriptor(data);
    if (fd < 0 || buffer == nullptr || actual == nullptr)
    {
    ...
    *actual = 0;
    ...
    while (size > 0)
    {
        const size_t to_write =
            static_cast<size_t>(std::min<uint64_t>(size, static_cast<uint64_t>(std::numeric_limits<size_t>::max())));
...
bool LocalFileSystemLinux::Write(FileEntryData& data, uint64_t size, const void* buffer, uint64_t* actual)
{
    const int fd = GetFileDescriptor(data);
    if (fd < 0 || buffer == nullptr || actual == nullptr)
    {
...
FileSize LocalFileSystemLinux::Size(const FileEntryData& data) const
{
    const int fd = GetFileDescriptor(data);
...
FileSystemError LocalFileSystemLinux::GetLastFileSystemError(bool operationSuccessful) const
{
    return ConvertToFileSystemError(GetAndUpdateLastError(operationSuccessful));
}

FileSystemError LocalFileSystemLinux::ConvertToFileSystemError(int error)
{
    switch (error)
    {
        case 0:
...
int LocalFileSystemLinux::GetAndUpdateLastError(bool operationSuccessful) const
{
