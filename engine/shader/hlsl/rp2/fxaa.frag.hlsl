// DX-B5: FXAA (ported from engine/shader/glsl/fxaa.frag).
...
float4 main(PSInput input) : SV_TARGET
{
    uint tex_w = 0;
    uint tex_h = 0;
    in_color.GetDimensions(tex_w, tex_h);
    float2 uv_step = float2(1.0 / max(tex_w, 1u), 1.0 / max(tex_h, 1u));

    float luma_mat[9];
    for (int i = 0; i < 9; ++i)
    {
        luma_mat[i] = RGB2LUMA(in_color.Sample(in_color_sampler, input.uv + uv_step * KERNEL_STEP_MAT[i]).rgb);
    }
    ...
    const float GRADIENT_SCALE = 0.25;
    ...
    float2 uv_start = input.uv + 0.5 * step_normal;
    ...
    float delta_luma_pos = RGB2LUMA(in_color.Sample(in_color_sampler, uv_pos).rgb) - luma_average_start;
    float delta_luma_neg = RGB2LUMA(in_color.Sample(in_color_sampler, uv_neg).rgb) - luma_average_start;
    ...
    if (!reached_both)
    {
        for (int i = 2; i < 12; ++i)
        {
            if (!reached_pos)
            {
                delta_luma_pos = RGB2LUMA(in_color.Sample(in_color_sampler, uv_pos).rgb) - luma_average_start;
            }
            if (!reached_neg)
            {
                delta_luma_neg = RGB2LUMA(in_color.Sample(in_color_sampler, uv_neg).rgb) - luma_average_start;
            }
    ...
    float length_pos = max(abs(uv_pos.x - uv_start.x), abs(uv_pos.y - uv_start.y));
    float length_neg = max(abs(uv_neg.x - uv_start.x), abs(uv_neg.y - uv_start.y));
    ...
    return in_color.Sample(in_color_sampler, input.uv + pixel_offset * step_normal);
}
