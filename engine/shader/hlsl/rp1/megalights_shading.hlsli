...
uint ml_light_count;
uint ml_tile_count_x;
uint ml_tile_count_y;
uint ml_num_samples;
uint ml_frame_index;
uint ml_ss_steps;
uint ml_viewport_width;
uint ml_viewport_height;
uint ml_temporal_enable;
uint ml_history_valid;
float ml_temporal_blend;
float ml_disocclusion_threshold;
uint ml_spatial_enable;
uint ml_spatial_radius;
float ml_spatial_depth_sigma;
float ml_spatial_normal_power;
...
void ML_LoadHeader()
{
    ml_light_count = ML_LoadUint(0);
    ml_tile_count_x = ML_LoadUint(4);
    ml_tile_count_y = ML_LoadUint(8);
    ml_num_samples = ML_LoadUint(12);
    ml_frame_index = ML_LoadUint(16);
    ml_ss_steps = ML_LoadUint(20);
    ml_viewport_width = ML_LoadUint(24);
    ml_viewport_height = ML_LoadUint(28);
    ml_temporal_enable = ML_LoadUint(32);
    ml_history_valid = ML_LoadUint(36);
    ml_temporal_blend = asfloat(ML_LoadUint(40));
    ml_disocclusion_threshold = asfloat(ML_LoadUint(44));
    ml_spatial_enable = ML_LoadUint(112);
    ml_spatial_radius = ML_LoadUint(116);
    ml_spatial_depth_sigma = asfloat(ML_LoadUint(120));
    ml_spatial_normal_power = asfloat(ML_LoadUint(124));
}
...
uint MegaLightsTileIndex(float2 texcoord)
{
    int2 tile_coord = int2(texcoord * float2((float)ml_tile_count_x, (float)ml_tile_count_y));
    tile_coord = clamp(tile_coord, int2(0, 0), int2((int)ml_tile_count_x - 1, (int)ml_tile_count_y - 1));
...
float MegaLightsLoadDepthAtUv(float2 uv, float fallback_depth)
{
    if (ml_viewport_width == 0u || ml_viewport_height == 0u)
    {
        return fallback_depth;
    }
    int2 sample_px = int2(uv * float2((float)ml_viewport_width, (float)ml_viewport_height));
    sample_px = clamp(sample_px, int2(0, 0), int2((int)ml_viewport_width - 1, (int)ml_viewport_height - 1));
    return in_scene_depth.Load(int3(sample_px, 0)).r;
}

float MegaLightsShadowSS(float3 world_pos, float3 light_pos, float2 uv, float scene_depth_at_pixel)
{
    ...
    for (int step_index = 1; step_index < steps; ++step_index)
    {
        ...
        float stored_depth = MegaLightsLoadDepthAtUv(sample_uv, scene_depth_at_pixel);
...
float3 MegaLightsStochasticDirect(float3 world_pos,
                                  ...
                                  float roughness)
{
    uint tile_index = MegaLightsTileIndex(texcoord);
    ...
    uint sample_count = min(ml_num_samples, tile_count);
    float3 accum = float3(0.0f, 0.0f, 0.0f);

    float2 noise = float2(MegaLightsHash(texcoord + float2((float)ml_frame_index * 0.1f, 0.0f)),
                          MegaLightsHash(texcoord.yx + float2((float)ml_frame_index * 0.07f, 0.0f)));
    ...
    for (uint sample_index = 0u; sample_index < sample_count; ++sample_index)
    {
        ...
        float visibility = MegaLightsShadowSS(world_pos, light_position, texcoord, scene_depth_at_pixel);
        float3 contribution =
                                 ...
                                 int3 pix)
{
    ...
    float depth_at_prev = MegaLightsLoadDepthAtUv(prev_uv, scene_depth_at_pixel);
    float4 prev_ndc_full = float4(UvToNdcxy(prev_uv), depth_at_prev, 1.0f);
    float4 world_from_prev_h = mul(inverse(prev_proj_view), prev_ndc_full);
    ...
    float3 lo_denoised = lerp(lo_raw, history, ml_temporal_blend);
